...
class HomeAssistant {
    ...
    static async testConnection(config) {
        const apiTest = new HaApi(config);
        try {
    ...
    async startListening ({ includeRegex, excludeRegex } = {}) {
        ...
        if (!isConnectionWorking) {
            throw new Error(
                `Connection to home assistant could not be established with config: ${this.config.baseUrl} ` +
                `${(this.config.apiPass) ? '<password redacted>' : '<password not used>'} ` +
                `${(this.config.accessToken) ? '<accessToken redacted>' : '<accessToken not used>'}`
            );
        }
